void caraRoja(int ejeY){
  pushMatrix();
  translate(597,ejeY);
  stroke(2);fill(0);point(0,0);noStroke();
  rotate(rr);
  

  for(int py = 0; py < 4; py = py + 1){
       int des = py * -13;
       color rojo= color(188,18,45);
       color violeta=color(143,45,130);
       if(mouseX>600){rojo =color(143,45,130);violeta=color(188,18,45);}
       for(int px = 0; px < 4; px = px + 1){ 
         if((px+py)%2==0){fill(rojo);}else{fill(violeta);}
         quad(px*26 + 13 + -65 + des,py*23 + -91,
         px*26 + 40 + -65 + des, py*23 + -91,
         px*26 + 27 + -65 + des, py*23 + -68,
         px*26 + -65 + des, py*23 + -68);
         
    }
  }
  
  pushMatrix();
  rotate(-2.09);
   for(int py = 0; py < 4; py = py + 1){
       color rojo= color(173,90,182);
       color violeta=color(216,50,60);
       if(mouseX>600){rojo =color(216,50,60);violeta=color(173,90,182);}
       int des = py * -13; 
       for(int px = 0; px < 4; px = px + 1){ 
         if((px+py)%2==0){fill(rojo);}else{fill(violeta);}
         quad(px*26 + 13 + -65 + des,py*23 + -91,
         px*26 + 40 + -65 + des, py*23 + -91,
         px*26 + 27 + -65 + des, py*23 + -68,
         px*26 + -65 + des, py*23 + -68);
         
    }
  }
 popMatrix();
   pushMatrix();
  rotate(2.10);
   for(int py = 0; py < 4; py = py + 1){
       color rojo= color(83,39,75);
       color violeta=color(120,36,49);
       if(mouseX>600){rojo =color(120,36,49);violeta=color(83,39,75);}
       int des = py * -13; 
       for(int px = 0; px < 4; px = px + 1){ 
         if((px+py)%2==0){fill(rojo);}else{fill(violeta);}
         quad(px*26 + 13 + -65 + des,py*23 + -91,
         px*26 + 40 + -65 + des, py*23 + -91,
         px*26 + 27 + -65 + des, py*23 + -68,
         px*26 + -65 + des, py*23 + -68);
         
    }
  }
 popMatrix();
 
 popMatrix();
 }
